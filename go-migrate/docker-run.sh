...
function mrcmd_func_go_migrate_docker_run() {
  ...
  mrcore_validate_value_required "DB URL" "${GO_MIGRATE_DB_DSN}"
  mrcmd_plugins_docker_validate_daemon_required

  # TODO: check GO_MIGRATE_DB_DSN if "?" exists before join GO_MIGRATE_DB_TABLE
  local goMigrateDbDsn="$(mrcore_lib_get_var_value "${GO_MIGRATE_DB_DSN}")&x-migrations-table=${GO_MIGRATE_DB_TABLE}"

  mrcore_echo_notice "GO_MIGRATE_DB_DSN: ${goMigrateDbDsn}"
