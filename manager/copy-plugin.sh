...
# run: mrcmd_plugins_call_function "manager/copy-plugin"
function mrcmd_func_manager_copy_plugin() {
  ...
  sourcePluginDir=$(manager_copy_plugin_get_plugin_dir "${sourcePluginName}")

  if [ ! -d "${sourcePluginDir}" ]; then
    mrcore_echo_error "The ${sourcePluginName} not found"
    ${EXIT_ERROR}
  fi
  ...
  if mrcore_lib_in_array "${destPluginDir}" MRCMD_PLUGINS_AVAILABLE_ARRAY[@] ; then
    mrcore_echo_error "The ${destPluginDir} plugin already exists"
    ${EXIT_ERROR}
  fi

  mrcore_validate_resource_exists "Dest plugin dir" "${destPluginDir}"

  manager_copy_plugin_copy "${sourcePluginName}" "${destPluginName}" "${sourcePluginDir}" "${destPluginDir}"
  manager_copy_plugin_rename "${sourcePluginName}" "${destPluginName}" "${destPluginDir}"

  mrcore_echo_ok "Plugin ${destPluginName} was copied from ${sourcePluginName}"
  mrcore_echo_sample "Run '${MRCMD_INFO_NAME} help ${destPluginName}' for usage"
}

# private
function manager_copy_plugin_copy() {
  local sourcePluginName="${1:?}"
  local destPluginName="${2:?}"
...
# private
function manager_copy_plugin_rename() {
  local sourcePluginName="${1:?}"
  local destPluginName="${2:?}"
  local pluginDir="${3:?}"
...
# private
function manager_copy_plugin_get_plugin_dir() {
