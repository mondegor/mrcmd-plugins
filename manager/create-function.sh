...
# run: mrcmd_plugins_call_function "manager/create-function"
function mrcmd_func_manager_create_function() {
  local pluginName="${1-}"
  local funcName="${2-}"
  local pluginDir="${APPX_PLUGINS_DIR}/${pluginName}"
  local templateFile="${MRCMD_CURRENT_PLUGINS_DIR}/${MRCMD_CURRENT_PLUGIN_NAME}/templates/function.sh.template"
  ...
  manager_create_function_exec "${pluginName}" "${funcName}" "${pluginDir}" "${templateFile}"

  mrcore_echo_ok "Function ${funcName} for plugin ${pluginName} was created"
  mrcore_echo_sample "Run '${MRCMD_INFO_NAME} ${pluginName} ${funcName}' for usage"
}

# private
function manager_create_function_exec() {
  local pluginName="${1:?}"
  local funcName="${2:?}"
  local pluginDir="${3:?}"
