...
## 2024-11-19
### Changed
- Добавлена команда завершения докера с закрытием его сокета;

## 2024-11-18
### Changed
- Для аргумента `DOCKER_IMAGE_FROM` в докер-файле добавлено значение по умолчанию;
- Обновлена версия `go`;

### Removed
- Удалён устаревший атрибут `version` из yaml файлов `docker-compose`;
- Удалён плагин `php-symfony`;

## 2024-10-11
### Added
- Подключён go линтер `gci` с возможностью установки, добавлены команды для него;

## 2024-10-09
### Added
- В `docker-compose`, выключен `traefik` во всех сервисов, в которых он не используется;
- Для `postgres` и `redis` добавлено прокидывание их TCP портов в `traefik`; 
- В `go` плагин добавлена новая команда `bench`;

## 2024-09-28
### Changed
- Переработаны плагины `go` и `go-dev`, обновлён список их команд,
  `go` плагин прокидывает `docker.sock`, что позволяет использовать `testcontainers`;
- В некоторых Dockerfile через EXPOSE прописаны необходимые порты;
- Также большинство публичных портов сервисов отключено, доступ к ним осуществляется через `traefic`;
- Поправлен `.editorconfig`, добавлены `*.proto`, `*.mk`;
- Исправлено название некоторых переменных; 

## 2024-08-03
### Changed
- Изменён лимит логирования на 14 дней.
...
## 2024-07-13
### Added
- Упростилась согласование сетей разных сервисов, теперь сеть создаётся
  через `docker`, а не `docker-compose`, для этого добавлены функции `remove-network`, `create-network`;

### Removed
- Удалена настройка `DEFAULT_SHELL` в `docker` плагине;
- обновлены версии некоторых докер образов;
...
- В большую часть плагинов добавлен метод `canexec`, который решает проблему поломки плагинов.
  Если этот метод возвращает false, то плагин начинает работать в ограниченном режиме (он предоставляет пользоваться только его переменными);

### Changed
- Обновление README.md, WIN_WSL;
- Для некоторых конструкций if добавлены двойные квадратные скобочки: `[[`;
- Благодаря добавленному плагину `traefik`, к панелям управления `kafka-ui`, `minio`, `mn-prometheus`
  теперь автоматически привязываются локальные домены;
- У плагина `go-migrate` появилась переменна `GO_MIGRATE_DB_TABLE`, с помощью которой можно переопределять
  название служебной таблицы, где хранятся версии миграций;
- У `plantuml` появилась команда генерации графических файлов из `puml` в отдельную директорию;
- `GO_LIB_DIR` переименована в `GO_GOPATH_DIR`;
- Обновлена конфигурация плагина `shellcheck`;
- Для плагина `global` добавлены две переменные `APPX_ENV` и `APPX_VER`. Для последней добавлен алгоритм
  определения версии приложения, если проект находится под `git`;
- В плагине `go-dev` прописано большое кол-во команд для удобства локальной разработки;

## 2024-03-02
### Added
- Добавлен плагин `pandoc` для конвертации `markdown` документов в `html` и `pdf` форматы;
- Добавлена библиотека для копирования текстовых файлов, имена и содержание которых заранее
  размечены специальными фразами. В момент копирования происходит замена этих фраз на нужные значения
  (используется для примитивной генерации); 

### Changed
- В `go-dev` плагине, если `$GOPATH` не указана, то сообщение об этом выводится только в режиме отладки;
- Оптимизирована производительность сборки `OpenAPI` документации,
  теперь она происходит через единый временный файл созданный через `mktemp`;
- В `nodejs` плагине докер теперь запускается в `root` режиме;
