...
## 2023-12-10
### Changed
- Добавлены изменения в сборке `OpenAPI` yaml файла: теперь можно указать отдельно версию `OpenAPI`
  (через переменную OPENAPI_VERSION) и собирать заголовки из нескольких файлов
  (OPENAPI_HEAD_PATH преобразовано в OPENAPI_HEADERS);
